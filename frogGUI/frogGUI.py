...
def graafik():
    ...
    if graf==True:
        ...
        x1 = np.arange(x0-10, x0+10, 1)#min max step
...
def veel():
    ...
    if t==0:
        ...
        btn_veel.config(text="Уменьшить")
        t=1
    else:
        aken.geometry(str(aken.winfo_width())+"x"+str(aken.winfo_height()-200))
        btn_veel.config(text="Увеличить")
        t=0
 
def kala():
   x1 = np.arange(0, 9.5, 1)
   y1=(2/27)*x1*x1-3
   x2 = np.arange(-10, 0.5, 1)
   y2=0.04*x2*x2-3
   x3 = np.arange(-9, -2.5, 1)
   y3=(2/9)*(x3+6)**2+1
   x4 = np.arange(-3, 9.5, 1)
   y4=(-1/12)*(x4-3)**2+6
   x5 = np.arange(5, 9, 1)
   y5=(1/9)*(x5-5)**2+2
   x6 = np.arange(5, 8.5,1)
   y6=(1/8)*(x6-7)**2+1.5
   x7 = np.arange(-13, -8.5,1)
   y7=(-0.75)*(x7+11)**2+6
   x8 = np.arange(-15, -12.5, 1)
   y8=(-0.5)*(x8+13)**2+3   
...
btn_veel=Button(f2,text="Увеличить",font ="Calibri 26",bg="green",command=veel)
btn_veel.pack(side=TOP)
 
var=IntVar()
r1=Radiobutton(f2,text="Whales",variable=var,var=1,font="Calibri 26",command=kala)
r2=Radiobutton(f2,text="Point", variable=var, var=2,font="Calibri 26")
r3=Radiobutton(f2,text="Frog",variable=var,var=3,font="Calibri 26")
